...
| B-004 fail-closed / no fallback | T001-T004 | client 构造/策略错误进入既有失败语义；sender `rg` guard |
| B-005 单一 SSOT | T001-T004 | 只引用 `ProviderHttpClient`/`ssrf_guard`；无自写 IP 分类、无新依赖 |
| B-006 全 sender + 兼容 | T001-T004 串行 | 四个 merge/PR gate 证据；合法公网 payload/header/signature/timeout/status 回归 |
