...
- [x] `SP831-T1` Owner: coordinator. Done when: `specs/GH831/product.md`, `tech.md`, `tasks.md` exist and pass SpecRail packet validation. Verify: `SPEC_RAIL=/Users/lifcc/Desktop/code/AI/tools/specrail; python3 "$SPEC_RAIL/checks/check_workflow.py" --repo "$SPEC_RAIL" --spec-dir "$PWD/specs/GH831"` passed.
...
- [x] `SP831-T7` Owner: coordinator. Done when: metric `gateway_unpriced_events_total{provider,model_bucket,policy,outcome}`、`gateway_unpriced_spend_total{provider,model_bucket,policy,outcome}` 与 error 日志字段就绪，reject preflight、routing candidate exclusion、settlement fallback 三条路径都记录，且 `model_bucket` 不使用任意请求 model. Verify: `cargo test metrics --all-features -- --nocapture` passed 386 tests, including middleware bounded-label render and app-factory `/metrics` endpoint coverage; `cargo test unpriced --all-features -- --nocapture` passed 20 tests, including reject preflight, candidate exclusion, and fallback settlement metrics.
- [x] `SP831-T8` Owner: verification owner. Done when: 全量回归、格式、lint、PR guard 通过，CHANGELOG 记录 breaking-behavior，并说明旧 `pricing.allow_degraded=true` 部署若要继续放行需显式设置 `unpriced_model_policy=allow_unpriced`. Verify: `cargo fmt --all -- --check` passed; `cargo check --all-features --message-format=short` passed; `cargo clippy --all-targets --all-features -- -D warnings` passed; `cargo test --all-features` passed 11044 lib tests, 5 main tests, all integration tests, and doctests 34 passed / 37 ignored; `bash scripts/guards/check_pr_scope.sh` and `bash scripts/guards/check_pr_overlap.sh` passed after commit.
...
- [x] `SP831-T9` Owner: verification owner. Done when: 复现测试证明默认配置下未定价模型请求被拒且预算不变、预算限制关闭/缺失时仍被 reject gate 拦截、allow 配置下预留被结算且记录带标记、fallback 按 usage 缩放、可定价 deployment 优先于未定价候选、cache hit 使用候选定价评估且不绕过 reject policy. Verify: `cargo test spend --all-features -- --nocapture` passed 90 lib tests + matched integration tests; `cargo test budget --all-features -- --nocapture` passed 248 lib tests + matched integration tests; `cargo test router --all-features -- --nocapture` passed 420 lib tests + matched integration tests; `cargo test cache --all-features -- --nocapture` passed 359 lib tests + matched integration tests.
