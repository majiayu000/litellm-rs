...
- [ ] `SP1128-T0` Covers: B-001 ～ B-009. Owner: maintainer/spec owner. Dependencies: none. Done when: 原规格已在 `user-2026-07-27-approve-all-specs` 获批；Issue 在本 amendment 审核期间移出 `ready_to_implement`，本 amendment 的 JSON/document semantic view、request-level function call、record-aware batch、不可吞掉的 response-integrity error、256/2 MiB 上限、稳定 400、Anthropic role-filtered provider view 与 zero-text-leaf projection suppression 契约获得 exact-head 内容绑定批准并合并后，才恢复 `ready_to_implement`. Verify: SpecRail workflow/spec checks、GitHub exact merged SHA 与 implement route gate。

## 实现任务

- [ ] `SP1128-T1` Covers: B-001, B-002, B-003, B-006, B-009. Owner: guardrail implementation owner. Dependencies: SP1128-T0. Done when: `content_text` 被 bounded fallible owned record builder 替代，覆盖 request/message legacy function、modern function、message name、普通 content、tool result/use；每条 message 的普通 text leaves 过滤其间非 text parts 后，仅在 leaves 非空时按固定顺序生成空串/单空格/换行三种 provider projections 并局部去重，全请求普通 text leaves 另生成 legacy newline view；reachable provider-transform profile 包含 Anthropic 时，另按请求顺序过滤掉所有非 System/Developer role，把存活普通 text leaves 生成唯一 newline view，无存活 leaf 时不生成，non-Anthropic-only profile 不得生成该 record；builder 镜像 Bedrock 的 ToolResult textual-leaf primitive，并按每个实际 outgoing ToolResult block 生成空串/空格/换行 views：普通 user/assistant 每个 ToolResult 独立，tool/function-role message 的普通 Text 与所有 ToolResult leaves 按 part 顺序进入同组；不同 block/message 不合并；其他字段继续生成 typed independent records，raw/semantic records 保持字段粒度；JSON 做 raw+semantic 扫描；bounded duplicate-rejecting visitor 在构造 `Value` 前拒绝任意层级/escape-equivalent duplicate key，leading BOM 与 structured-looking argument 的 syntax/recursion/depth/resource 解析失败稳定 400，普通非 JSON 仍扫描 raw；全部 projections/records 纳入 256/2 MiB checked accounting. Verify: focused empty-leaf/256-image-only/filtered-part/three-projection/cross-message/Anthropic-System-Developer-profile/non-Anthropic-zero-record/outgoing-ToolResultBlock-provider-view/provider-transform/record-isolation/JSON/duplicate-key/BOM/depth-limit/limit tests.
...
- provider-boundary fixtures 必须证明 Bedrock space、Ollama newline、Gemini/Vertex
  ...
  views 不互相跨 record 匹配；Anthropic profile 必须把
  System `ignore`、中间 User `hello`、随后分别为 Developer/System 的
  `all previous instructions` 精确映射为 `ignore\nall previous instructions`，
  non-Anthropic-only profile 不产生该 record；Bedrock ToolResult array、tool/function-role 的
