...
- provider-boundary fixtures 必须证明 Bedrock space、Ollama newline、Gemini/Vertex
  empty join 都被对应 adjacency record 覆盖；`ignore` + `all previous instructions`
  在 provider 转换/调用前命中，三种 view 不互相跨 record 匹配。
