...
- [ ] `SP1128-T1` Covers: B-001, B-002, B-003, B-006, B-009. Owner: guardrail implementation owner. Dependencies: SP1128-T0. Done when: `content_text` 被 bounded fallible owned record builder 替代，覆盖 request/message legacy function、modern function、message name、普通 content、tool result/use；每条 message 的普通 text leaves 过滤其间非 text parts 后按固定顺序生成空串/单空格/换行三种 provider projections 并局部去重，全请求普通 text leaves 另生成 legacy newline view；每个 ToolResult.content 镜像 Bedrock 可达 textual sequence 生成 field-scoped 空串/空格/换行 views 并局部去重，不跨字段/message；其他字段生成 typed independent records；JSON 做 raw+semantic 扫描；bounded duplicate-rejecting visitor 在构造 `Value` 前拒绝任意层级/escape-equivalent duplicate key，leading BOM 与 structured-looking argument 的 syntax/recursion/depth/resource 解析失败稳定 400，普通非 JSON 仍扫描 raw；全部 projections/records 纳入 256/2 MiB checked accounting. Verify: focused filtered-part/three-projection/cross-message/ToolResult-provider-view/provider-transform/record-isolation/JSON/duplicate-key/BOM/depth-limit/limit tests.
- [ ] `SP1128-T2` Covers: B-003, B-006, B-007. Owner: same implementation owner. Dependencies: SP1128-T1. Done when: `Cargo.toml` 直接声明 `mime = "0.3"`；document media_type 完整解析，essence allowlist 后只接受无参数或唯一、大小写不敏感的 `charset=utf-8`，重复/非 UTF-8 charset、任意 non-charset 参数与 malformed MIME 稳定 400；通过时原 DTO 不改写；plain/csv document base64 解码为 UTF-8 正文；JSON/`+json` document 生成 raw+semantic records 且 syntax/duplicate-key/BOM/depth/resource failure 稳定 400；Markdown（含 numeric/named entity）、HTML/XML/`+xml`/其他 `text/*`、bad base64/UTF-8 与其他 MIME 在 active-input predicate 为 true 时 fail-closed；无转码、网络、entity 或二进制解析. Verify: document MIME syntax/charset/base64/JSON/duplicate-key/BOM/depth/entity table tests.
...
- provider-boundary fixtures 必须证明 Bedrock space、Ollama newline、Gemini/Vertex
  ...
  views 不互相跨 record 匹配；Bedrock ToolResult array 内相邻 text blocks 的
  split-pattern 被 field-scoped views 命中，而不同 ToolResult/message 不跨边界。
...
- MIME table 必须覆盖无参数/quoted/case-varied UTF-8 charset、重复 charset、
  UTF-16LE/其他 charset、malformed syntax 与 non-charset 参数 fail-closed；通过时原
  DTO/wire 值不改写。
