...
- [ ] `SP1127-T1` Covers: B-001, B-002, B-003, B-005, B-008, B-012, B-013, B-014, B-015. Owner: stream guardrail implementation owner. Dependencies: SP1127-T0. Done when: config 与 `check_output_text`、共享 `StreamOutputGuard` 完成 fixed windowed cumulative、256/1..=4096、8 MiB 双上限、全部客户端可见文本 surface、Responses/reasoning/logprobs 去重、按 choice 跨 event 维护且不可逆物化的 content/chosen alias 状态、按 `(choice.index, tool_call.index, field)` 维护 Chat parallel tool-call 连续 surface、UTF-8 安全和 disabled fast path. Verify: focused config/guardrail/state-machine tests.
