...
- [ ] `SP1127-T1` Covers: B-001, B-002, B-003, B-005, B-008, B-012, B-013, B-014, B-015. Owner: stream guardrail implementation owner. Dependencies: SP1127-T0. Done when: config 与 `check_output_text`、共享 `StreamOutputGuard` 完成 fixed windowed cumulative、256/1..=4096、8 MiB 双上限、全部客户端可见文本 surface、Responses/reasoning/logprobs 去重、按 choice 跨 event 维护且不可逆物化的 content/chosen alias 状态、top candidates 按 `(choice.index, candidate_index)` 跨 position/event 连续累计且不做逐位置 chosen skip、按 `(choice.index, tool_call.index, field)` 维护 Chat parallel tool-call 连续 surface、UTF-8 安全和 disabled fast path. Verify: focused config/guardrail/state-machine tests.
...
- [ ] `SP1127-T3` Covers: B-002 ～ B-016. Owner: test owner. Dependencies: SP1127-T2. Done when: 单/多窗口、后续窗口违规、跨 chunk、UTF-8、Chat content/thinking/reasoning/audio transcript/logprobs/tool/function、parallel tool calls 交错 `call0:sec`/`call1:x`/`call0:ret` 的连续与隔离、Completion text/echo/logprobs token/refusal、Responses output/reasoning/function surface、Responses initial/late/repeated name、late-name update 后 provider error 不扫描、late name 在 output-item-done 前检查、state 已存在时 accepted arguments、pre-ID dropped arguments 不扫描、done/snapshot 去重、Chat alias/token 去重，以及 chosen tokens 跨 token 拼成敏感词、前一 event `content/chosen=sec` 后一 event chosen-only `ret` 仍命中 `secret` 的不可逆 alias 物化、top candidate 同一 rank 跨 position/event 的 `sec`/`ret` 命中（含首位置 top 等于 chosen 后分叉）、guardrail timeout/error/fail_open、overflow、pending 后 provider error/idle 不泄露、disconnect 和 lifecycle exactly-once fixture 完整；不得用不可从 canonical provider pipeline 到达的 synthetic Chat refusal 声称覆盖. Verify: focused route/lifecycle test filters.
