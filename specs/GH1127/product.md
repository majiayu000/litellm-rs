...
13. B-013 `/v1/responses` 的 output text、reasoning summary、function-call name 与 arguments 都属于客户端可见模型文本，必须从上游语义 delta 恰好一次进入累计扫描序列；`.done` events、output items 与最终 response snapshot 不得重复累计同一文本。
