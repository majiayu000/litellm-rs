...
- [ ] `SP1035-T2` Covers: B-001, B-002, B-003. Owner: implementation owner. Dependencies: SP1035-T1. Done when: 精确 Cargo update 将唯一 `spin 0.9.8` package 更新到 `0.9.9`，工作树只修改 `Cargo.lock`；diff 只含 package version/checksum 与 `flume`、`lazy_static` 既有 dependency entry 的版本消歧引用，不改变其他 package 版本或依赖集合. Verify: `cargo update -p spin@0.9.8 --precise 0.9.9`; `git diff --stat`; `git diff -- Cargo.lock`; `cargo update -p spin@0.9.8 --precise 0.9.9 --dry-run` 预期无变化.
