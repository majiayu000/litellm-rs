...
- [ ] `SP838-T6` Owner: coordinator. Done when: gate lane 执行完毕——被 gate 模块默认不编译，README、`docs/README.md`、相关当前能力 docs。Verify: 见下列命令与行为证据。
      ...
      `git ls-files README.md CLAUDE.md config src/config docs/README.md 'docs/protocols/**/*.md' | xargs rg -ni "MCP Gateway|A2A Protocol|A2A Gateway|A2AGateway|Model Context Protocol|mcp|a2a|realtime|webhooks|user_management"`
      同时覆盖全部五个 gated name 的当前能力 docs/config surfaces，每个命中都与 default-off experimental 处置一致且无默认/稳定能力宣称；历史 docs/plan/specs 不作为阻塞，已删除 protocol 文件不会使命令失败。
...
- [ ] `SP838-T7c` Owner: remove-compatibility owner. Dependencies: T3. Done when: 0.6.x compatibility/deprecation tranche 为 `semantic_cache` 与 `analytics` public modules 添加 deprecation，补齐 CHANGELOG 与迁移说明；保留 `litellm_rs::core::{semantic_cache,analytics}` public imports，保留 `cache.semantic_cache` / `enterprise.advanced_analytics` 现有 config parse/rejection/runtime 行为，且保留 `analytics`、`enterprise`、`full`、docs.rs 现有 Cargo feature surface，不在 0.6.x 提前删除；新增的 semantic_cache/analytics assertions 在 `providers-extended,analytics` feature set 下不得被任何其他 `cfg` 剔除。Verify: 扩展 `tests/public_api_compat.rs` 覆盖两个 public import；config compatibility fixtures；`cargo test --test public_api_compat --features "providers-extended,analytics"` 必须编译并执行新增 assertions；`cargo check --no-default-features --features analytics`；`cargo check --features enterprise`；`cargo check --features full`；deprecation/CHANGELOG/migration diff；`git diff --check`。
- [ ] `SP838-T7b` Owner: batch removal owner. Dependencies: T7a, T7v；0.6 deprecation 已有可验证 release artifact. Done when: 0.7.0 breaking tranche 仅删除公开 `BatchProcessor` 及其专属实现，清理对应 `core/mod.rs`/文档/CHANGELOG/migration 引用，同步移除或改写 `src/core/subsystem_registry.rs` 中 stale `BatchProcessor` runtime/export claim 并更新对应 guard tests/exemptions，同时保留 `/v1/batches` provider proxy、`AsyncBatchExecutor`、共享 batch 类型、database schema/history；任何扩大 removal scope 的变化必须先有独立 spec 批准. Verify: `SP838-T7v` fixture 证据与含 `BatchProcessor` deprecation 的已验证 0.6 release artifact；subsystem registry/guard regression 无 stale claim；`cargo check --all-features`; `cargo test --all-features`; proxy 回归；public removal compile fixture；`git diff --check`。
- [ ] `SP838-T7` Owner: coordinator. Dependencies: T7v, T7a, T7b, T7c；所有 remove 行的 deprecation 已有可验证 0.6 release artifact. Done when: 批准的完整 remove lane（仅矩阵中 `remove` 行）均已执行。Verify: 见下列命令与行为证据。
      `core/mod.rs`、`src/core/subsystem_registry.rs` 及对应 guard tests/exemptions、config schema/env/example、README、CLAUDE.md、`docs/README.md`、相关当前能力 docs（只扫现存 tracked docs）
      同步清理；batch/semantic_cache/analytics 不得留下 stale registry/runtime/export claim；所有 public removal 均满足各自 semver、CHANGELOG、deprecation/迁移与 release gate；analytics removal 同时删除
      ...
      仅检查 remove 行的 config/schema/examples、当前能力 docs 与迁移说明，历史 docs/plan/specs 不作为阻塞；`rg -n 'batch|semantic_cache|analytics|BatchProcessor|SemanticCache|AnalyticsEngine' src/core/subsystem_registry.rs src/core/subsystem_registry/tests.rs` 输出与 removal 后真实 export/runtime 一致且 guard tests 无 stale exemption；public import/feature 破坏有发布记录。
