...
- [ ] `SP969-T2` Covers: B-006, B-007. Owner: implementation owner. Dependencies: SP969-T1. Done when: `check_log_pii.sh` 使用独立零基线扫描 log macro 中的 `session_id|session_token|sid`，并在当前三处泄漏仍存在时精确红灯；两套 lint workflow 调用该 guard. Verify: `bash scripts/guards/check_log_pii.sh` 预期非零且输出三个 known paths；`LITELLM_LOG_PII_BASELINE_MAX=999 bash scripts/guards/check_log_pii.sh` 仍预期非零；`rg -n "Log PII guard|check_log_pii" .github/workflows/ci.yml .github/workflows/ci-main-full.yml`.
