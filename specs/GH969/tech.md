...
2. 扩展 `scripts/guards/check_log_pii.sh`：
   ...
   - 新增独立 token-tree scan，忽略注释并按平衡的 `()`、`[]`、`{}` 确定 macro 调用边界；只解析宏顶层
     实际 format-template segment，cooked string 先解码 Rust escape/续行、raw string 保持原文，再查找
     `session_id`、`session_token` 或 `sid` 的直接或隐式引用；扫描不得依赖 Rust 语句末尾分号；
...
- [ ] Parser regression: scanner self-test 必须捕获字符串内分号、tail/match-arm、多行嵌套调用、普通/原始
  format capture、cooked escape/续行与 format specifier 引用，同时忽略 prose、转义 braces、target/data
