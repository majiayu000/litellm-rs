...
impl<'ast> Visit<'ast> for DefaultFinder<'_> {
        ...
        } fn visit_expr_call(&mut self, expression: &'ast syn::ExprCall) { let segments = match &*expression.func { Expr::Path(path) => path.path.segments.iter().map(|segment| ident_text(&segment.ident)).collect::<Vec<_>>(), _ => Vec::new() }; if segments.last().is_some_and(|name| name == "default") { self.0 = true; } else if !segments.windows(2).any(|window| window == ["GlobalPoolManager", "new_for_provider"]) { expression.args.iter().for_each(|argument| self.visit_expr(argument)); } } fn visit_expr_method_call(&mut self, expression: &'ast ExprMethodCall) { let method = ident_text(&expression.method); if matches!(method.as_str(), "unwrap_or_default" | "or_default" | "get_or_insert_default") { self.0 = true; } else { self.visit_expr(&expression.receiver); expression.args.iter().for_each(|argument| self.visit_expr(argument)); } } fn visit_macro(&mut self, item: &'ast Macro) { let name = item.path.segments.last().map(|segment| ident_text(&segment.ident)); let tokens = item.tokens.to_string(); self.0 |= name.is_some_and(|name| self.2.contains(&name)) || tokens.to_ascii_lowercase().contains("default") || self.1.iter().any(|local| tokens.split(|character: char| !(character == '_' || character.is_alphanumeric())).any(|token| token == local)); } }
...
#[rustfmt::skip] fn return_uses_default(expr: &Expr, variants: &[String], locals: &[String], macros: &[String]) -> bool { if let Expr::Path(path) = expr && path.qself.is_none() && path.path.segments.len() == 1 { return path.path.segments.first().is_some_and(|segment| locals.contains(&ident_text(&segment.ident))); } else if variants.is_empty() { return uses_default_or_local(expr, locals, macros); } let Expr::Call(call) = expr else { return false; }; let Expr::Path(path) = &*call.func else { return false; }; path.path.segments.last().is_some_and(|segment| variants.contains(&ident_text(&segment.ident))) && call.args.iter().any(|argument| uses_default_or_local(argument, locals, macros)) }
...
#[rustfmt::skip] fn default_macro_names(file: &syn::File) -> Vec<String> { struct Macros(Vec<(String, String)>); impl<'ast> Visit<'ast> for Macros { fn visit_item_macro(&mut self, item: &'ast ItemMacro) { if let Some(name) = &item.ident { self.0.push((ident_text(name), item.mac.tokens.to_string())); } visit::visit_item_macro(self, item); } } let mut macros = Macros(Vec::new()); macros.visit_file(file); let mut defaults = Vec::new(); loop { let mut changed = false; for (name, tokens) in &macros.0 { let compact = tokens.replace(' ', ""); let direct = compact.to_ascii_lowercase().contains("default") && !compact.contains("GlobalPoolManager::new_for_provider"); let transitive = defaults.iter().any(|default: &String| tokens.split(|character: char| !(character == '_' || character.is_alphanumeric())).any(|token| token == default)); if (direct || transitive) && !defaults.contains(name) { defaults.push(name.clone()); changed = true; } } if !changed { return defaults; } } }
