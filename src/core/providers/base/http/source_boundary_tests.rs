...
#[rustfmt::skip] fn root_container(expr: &Expr, aliases: &[String]) -> bool { match expr { Expr::Call(call) => matches!(&*call.func, Expr::Path(path) if path.path.segments.last().is_some_and(|segment| matches!(canonical_variant(ident_text(&segment.ident), aliases).as_str(), "Ok" | "Err" | "Some" | "None"))), Expr::MethodCall(call) => matches!(ident_text(&call.method).as_str(), "map" | "map_err" | "and_then" | "or_else" | "ok" | "err" | "ok_or" | "ok_or_else" | "transpose"), Expr::Group(group) => root_container(&group.expr, aliases), Expr::Paren(paren) => root_container(&paren.expr, aliases), Expr::Try(value) => root_container(&value.expr, aliases), Expr::Block(block) => block.block.stmts.last().is_some_and(|stmt| matches!(stmt, syn::Stmt::Expr(expr, None) if root_container(expr, aliases))), _ => false } }
#[rustfmt::skip] fn variant_value<'a>(expr: &'a Expr, expected: &str, aliases: &[String]) -> Option<&'a Expr> { match expr { Expr::Group(group) => variant_value(&group.expr, expected, aliases), Expr::Paren(paren) => variant_value(&paren.expr, expected, aliases), Expr::Call(call) if matches!(&*call.func, Expr::Path(path) if path.path.segments.last().is_some_and(|segment| canonical_variant(ident_text(&segment.ident), aliases) == expected)) => call.args.first(), _ => None } }
#[rustfmt::skip] fn transpose_uses_default(expr: &Expr, variants: &[String], locals: &[String], aliases: &[String]) -> bool { let layers = if variants.contains(&"Ok".into()) { Some(("Some", "Ok")) } else if variants.contains(&"Err".into()) { Some(("Some", "Err")) } else if variants.contains(&"Some".into()) { Some(("Ok", "Some")) } else { None }; layers.and_then(|(outer, inner)| variant_value(expr, outer, aliases).and_then(|value| variant_value(value, inner, aliases))).is_some_and(|value| uses_default_or_local(value, locals, aliases)) }
#[rustfmt::skip] struct ReturnDefaultFinder<'a>(bool, &'a [String], &'a [String], &'a [String]);
#[rustfmt::skip] #[allow(clippy::possible_missing_else)] impl<'ast> Visit<'ast> for ReturnDefaultFinder<'_> { fn visit_expr(&mut self, expr: &'ast Expr) { if !expr_attrs(expr).is_some_and(has_test_cfg) { visit::visit_expr(self, expr); } } fn visit_path(&mut self, path: &'ast Path) { if path.segments.len() == 1 { self.0 |= path.segments.first().is_some_and(|segment| self.2.contains(&ident_text(&segment.ident))); } } fn visit_expr_call(&mut self, expression: &'ast syn::ExprCall) { let variant = match &*expression.func { Expr::Path(path) => path.path.segments.last().map(|segment| canonical_variant(ident_text(&segment.ident), self.3)), _ => None }; if variant.as_ref().is_some_and(|name| matches!(name.as_str(), "Ok" | "Err" | "Some" | "None")) { if variant.is_some_and(|name| self.1.contains(&name)) { self.0 |= expression.args.iter().any(|argument| uses_default_or_local(argument, self.2, self.3)); } } else { self.visit_expr(&expression.func); expression.args.iter().for_each(|argument| self.visit_expr(argument)); } } fn visit_expr_method_call(&mut self, expression: &'ast ExprMethodCall) { let method = ident_text(&expression.method); if method == "transpose" { self.0 |= transpose_uses_default(&expression.receiver, self.1, self.2, self.3); return; } let alternate = if matches!(method.as_str(), "ok_or" | "ok_or_else") && self.1.contains(&"Ok".into()) { Some(vec!["Some".into()]) } else if method == "ok" && self.1.contains(&"Some".into()) { Some(vec!["Ok".into()]) } else if method == "err" && self.1.contains(&"Some".into()) { Some(vec!["Err".into()]) } else { None }; if let Some(variants) = alternate { let mut finder = ReturnDefaultFinder(false, &variants, self.2, self.3); finder.visit_expr(&expression.receiver); self.0 |= finder.0; } else { self.visit_expr(&expression.receiver); } if matches!(method.as_str(), "map" | "and_then" | "or_else") { self.0 |= expression.args.iter().any(|argument| uses_default_or_local(argument, self.2, self.3)); } } fn visit_macro(&mut self, item: &'ast Macro) { let name = item.path.segments.last().map(|segment| ident_text(&segment.ident)); let tokens = item.tokens.to_string(); self.0 |= name.is_some_and(|name| self.3.contains(&name)) || tokens.split(|character: char| !(character == '_' || character.is_alphanumeric())).any(|token| self.3.iter().any(|name| name == token)); } }
...
#[rustfmt::skip] fn manager_place(expr: &Expr, fields: &[String], locals: &[String]) -> bool { match expr { Expr::Reference(reference) => manager_place(&reference.expr, fields, locals), Expr::Paren(paren) => manager_place(&paren.expr, fields, locals), Expr::Group(group) => manager_place(&group.expr, fields, locals), Expr::Unary(unary) if matches!(unary.op, syn::UnOp::Deref(_)) => manager_place(&unary.expr, fields, locals), Expr::Path(_) => path_name(expr).is_some_and(|name| locals.contains(&name)), Expr::Field(field) => match &field.member { syn::Member::Named(name) => fields.contains(&ident_text(name)), syn::Member::Unnamed(index) => path_name(&field.base).is_some_and(|name| locals.contains(&format!("{name}#{}", index.index))) }, _ => false } }
...
#[rustfmt::skip] fn collect_renames(tree: &UseTree, prefix: &mut Vec<String>, renames: &mut Vec<(Vec<String>, String)>) { match tree { UseTree::Path(path) => { prefix.push(ident_text(&path.ident)); collect_renames(&path.tree, prefix, renames); prefix.pop(); }, UseTree::Rename(rename) => { let mut path = prefix.clone(); path.push(ident_text(&rename.ident)); renames.push((path, ident_text(&rename.rename))); }, UseTree::Group(group) => group.items.iter().for_each(|tree| collect_renames(tree, prefix, renames)), _ => {} } }
...
fn manager_fields(source: &str) -> Result<Vec<String>, syn::Error> {
    struct Fields(Vec<String>); impl<'ast> Visit<'ast> for Fields { fn visit_item(&mut self, item: &'ast syn::Item) { if !item_attrs(item).is_some_and(has_test_cfg) { visit::visit_item(self, item); } } fn visit_item_struct(&mut self, item: &'ast ItemStruct) { self.0.extend(item.fields.iter().enumerate().filter(|(_, field)| !has_test_cfg(&field.attrs) && is_manager_type(&field.ty)).map(|(index, field)| field.ident.as_ref().map_or_else(|| format!("{}#{index}", item.ident), ident_text))); visit::visit_item_struct(self, item); } fn visit_item_enum(&mut self, item: &'ast syn::ItemEnum) { for variant in &item.variants { self.0.extend(variant.fields.iter().enumerate().filter(|(_, field)| !has_test_cfg(&field.attrs) && is_manager_type(&field.ty)).map(|(index, field)| field.ident.as_ref().map_or_else(|| format!("{}#{index}", variant.ident), ident_text))); } visit::visit_item_enum(self, item); } }
...
impl BoundaryVisitor {
    ...
    #[rustfmt::skip]    fn add_aliases(&mut self, tree: &UseTree) { let mut aliases = self.default_macros.clone(); extend_aliases(tree, &mut aliases); for alias in aliases { self.add_alias(alias); } let mut renames = Vec::new(); collect_renames(tree, &mut Vec::new(), &mut renames); for (path, alias) in renames { let Some(original) = path.last() else { continue; }; if path.len() == 1 || (path.len() == 2 && path[0] == "self") { for marker in self.manager_fields.clone().into_iter().filter(|marker| marker.starts_with(&format!("{original}#"))) { let marker = marker.replacen(original, &alias, 1); if !self.manager_fields.contains(&marker) { if let Some(scope) = self.alias_scopes.last_mut() { scope.push(format!("@manager:{marker}")); } self.manager_fields.push(marker); } } } } }
