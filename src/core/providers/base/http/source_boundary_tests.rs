...
#[rustfmt::skip] fn returns_manager(output: &ReturnType) -> bool { matches!(output, ReturnType::Type(_, ty) if is_manager_type(ty)) }
...
#[rustfmt::skip] fn variant_value<'a>(expr: &'a Expr, expected: &str, aliases: &[String]) -> Option<&'a Expr> { match expr { Expr::Group(group) => variant_value(&group.expr, expected, aliases), Expr::Paren(paren) => variant_value(&paren.expr, expected, aliases), Expr::Block(block) => block.block.stmts.last().and_then(|stmt| match stmt { syn::Stmt::Expr(expr, None) => variant_value(expr, expected, aliases), _ => None }), Expr::Call(call) if matches!(&*call.func, Expr::Path(path) if path.path.segments.last().is_some_and(|segment| canonical_variant(ident_text(&segment.ident), aliases) == expected)) => call.args.first(), _ => None } }
#[rustfmt::skip] fn transpose_uses_default(expr: &Expr, variants: &[String], locals: &[String], aliases: &[String]) -> bool { variants.iter().any(|variant| { let output: Vec<_> = variant.split('/').filter(|part| !part.is_empty()).collect(); let input: Vec<_> = match output.as_slice() { ["Some", "Ok", rest @ ..] => [vec!["Ok", "Some"], rest.to_vec()].concat(), ["Some", "Err", rest @ ..] => [vec!["Err"], rest.to_vec()].concat(), ["Ok", "Some", rest @ ..] => [vec!["Some", "Ok"], rest.to_vec()].concat(), ["Err", rest @ ..] => [vec!["Some", "Err"], rest.to_vec()].concat(), _ => Vec::new() }; input.into_iter().try_fold(expr, |value, layer| variant_value(value, layer, aliases)).is_some_and(|value| uses_default_or_local(value, locals, aliases)) }) }
...
#[rustfmt::skip] fn type_name(ty: &Type) -> Option<String> { match ty { Type::Path(path) => path.path.segments.last().map(|segment| ident_text(&segment.ident)), Type::Group(group) => type_name(&group.elem), Type::Paren(paren) => type_name(&paren.elem), Type::Reference(reference) => type_name(&reference.elem), _ => None } }
#[rustfmt::skip] fn typed_manager_names(pat: &Pat, ty: &Type, fields: &[String]) -> Vec<String> { let names = pat_names(pat); if is_manager_type(ty) { return names; } let Some(kind) = type_name(ty) else { return Vec::new(); }; names.into_iter().flat_map(|name| fields.iter().filter_map({ let kind = kind.clone(); move |field| field.strip_prefix(&format!("{kind}#")).map(|index| format!("{name}#{index}")) })).collect() }
#[rustfmt::skip] fn signature_manager_names(signature: &syn::Signature, fields: &[String]) -> Vec<String> { signature.inputs.iter().flat_map(|argument| match argument { syn::FnArg::Typed(argument) => typed_manager_names(&argument.pat, &argument.ty, fields), syn::FnArg::Receiver(_) => Vec::new() }).collect() }
#[rustfmt::skip] fn path_name(expr: &Expr) -> Option<String> { match expr { Expr::Path(path) if path.qself.is_none() && path.path.segments.len() == 1 => path.path.segments.first().map(|segment| ident_text(&segment.ident)), Expr::Paren(paren) => path_name(&paren.expr), Expr::Group(group) => path_name(&group.expr), Expr::Reference(reference) => path_name(&reference.expr), Expr::Unary(unary) if matches!(unary.op, syn::UnOp::Deref(_)) => path_name(&unary.expr), _ => None } }
#[rustfmt::skip] fn manager_place(expr: &Expr, fields: &[String], locals: &[String]) -> bool { match expr { Expr::Reference(reference) => manager_place(&reference.expr, fields, locals), Expr::Paren(paren) => manager_place(&paren.expr, fields, locals), Expr::Group(group) => manager_place(&group.expr, fields, locals), Expr::Unary(unary) if matches!(unary.op, syn::UnOp::Deref(_)) => manager_place(&unary.expr, fields, locals), Expr::Try(value) => manager_place(&value.expr, fields, locals), Expr::Await(value) => manager_place(&value.base, fields, locals), Expr::Block(block) => block.block.stmts.last().is_some_and(|stmt| matches!(stmt, syn::Stmt::Expr(expr, None) if manager_place(expr, fields, locals))), Expr::Path(_) => path_name(expr).is_some_and(|name| locals.contains(&name)), Expr::Field(field) => match &field.member { syn::Member::Named(name) => fields.contains(&ident_text(name)), syn::Member::Unnamed(index) => path_name(&field.base).is_some_and(|name| locals.contains(&format!("{name}#{}", index.index))) }, Expr::MethodCall(call) if call.args.is_empty() && matches!(ident_text(&call.method).as_str(), "clone" | "to_owned") => manager_place(&call.receiver, fields, locals), Expr::Call(call) if matches!(&*call.func, Expr::Path(path) if path.path.segments.last().is_some_and(|segment| segment.ident == "identity")) => call.args.iter().any(|arg| manager_place(arg, fields, locals)), Expr::Tuple(tuple) => tuple.elems.iter().any(|value| manager_place(value, fields, locals)), Expr::Match(value) => value.arms.iter().any(|arm| manager_place(&arm.body, fields, locals)), Expr::If(value) => value.then_branch.stmts.last().is_some_and(|stmt| matches!(stmt, syn::Stmt::Expr(expr, None) if manager_place(expr, fields, locals))) || value.else_branch.as_ref().is_some_and(|(_, expr)| manager_place(expr, fields, locals)), _ => false } }
...
impl BoundaryVisitor {
    ...
    #[rustfmt::skip]    fn push_pattern(&mut self, pat: &Pat, default_tainted: bool, manager_tainted: bool) { self.local_scopes.push(Vec::new()); self.alias_scopes.push(Vec::new()); let names = pat_names(pat); self.shadow_aliases(&names); for name in names { let (was_default, was_manager) = (self.default_locals.contains(&name), self.manager_locals.contains(&name)); let Some(scope) = self.local_scopes.last_mut() else { panic!("pattern scope must exist") }; scope.push((name.clone(), was_default, was_manager)); self.default_locals.retain(|local| local != &name); self.manager_locals.retain(|local| local != &name); if default_tainted { self.default_locals.push(name.clone()); } if manager_tainted { self.manager_locals.push(name); } } }
    ...
    #[rustfmt::skip]    fn pop_pattern(&mut self) { self.pop_alias_scope(); self.pop_scope(); }
...
impl<'ast> Visit<'ast> for BoundaryVisitor {
    ...
    fn visit_expr_call(&mut self, expression: &'ast syn::ExprCall) { if let Expr::Path(path) = &*expression.func { let segments = path.path.segments.iter().map(|segment| ident_text(&segment.ident)).collect::<Vec<_>>(); if let Some(constructor) = segments.last() { if is_take_path(&segments, &self.default_macros) && expression.args.iter().any(|argument| manager_place(argument, &self.manager_fields, &self.manager_locals)) { self.record("implicit Default construction for GlobalPoolManager via mem::take".into()); } let canonical = resolve_relative_path(&self.module_path, &segments).map(|path| path.join("::")); for (index, argument) in expression.args.iter().enumerate() { if (self.manager_fields.contains(&format!("{constructor}#{index}")) || canonical.as_ref().is_some_and(|path| self.manager_fields.contains(&format!("{path}#{index}")))) && uses_default_or_local(argument, &self.default_locals, &self.default_macros) { self.record(format!("policy-less Default construction for GlobalPoolManager tuple field {constructor}#{index}")); } } } } visit::visit_expr_call(self, expression); }
    fn visit_local(&mut self, local: &'ast Local) { if has_test_cfg(&local.attrs) { return; } let names = pat_names(&local.pat); let inferred_default = local.init.as_ref().is_some_and(|init| if self.manager_returning && root_container(&init.expr, &self.default_macros) { return_uses_default(&init.expr, &self.return_variants, &self.default_locals, &self.default_macros) } else { uses_default_or_local(&init.expr, &self.default_locals, &self.default_macros) }); let inferred_manager = local.init.as_ref().is_some_and(|init| manager_place(&init.expr, &self.manager_fields, &self.manager_locals)); let typed = match &local.pat { Pat::Type(pat) => typed_manager_names(&pat.pat, &pat.ty, &self.manager_fields), _ => Vec::new() }; for name in &names { let (was_default, was_manager) = (self.default_locals.contains(name), self.manager_locals.contains(name)); if let Some(scope) = self.local_scopes.last_mut() { scope.push((name.clone(), was_default, was_manager)); } self.default_locals.retain(|local| local != name); self.manager_locals.retain(|local| local != name && !local.starts_with(&format!("{name}#"))); } self.shadow_aliases(&names); let manager = inferred_manager || match &local.pat { Pat::Type(pat) => is_manager_type(&pat.ty), Pat::Ident(pat) => self.manager_fields.contains(&ident_text(&pat.ident)), _ => false }; self.manager_locals.extend(typed); if manager { self.manager_locals.extend(names.iter().cloned()); } if inferred_default { if manager { self.record("policy-less Default construction for GlobalPoolManager local".into()); } self.default_locals.extend(names); } visit::visit_local(self, local); }
    ...
    fn visit_expr_match(&mut self, expression: &'ast syn::ExprMatch) { let default_tainted = uses_default_or_local(&expression.expr, &self.default_locals, &self.default_macros); let manager_tainted = manager_place(&expression.expr, &self.manager_fields, &self.manager_locals); self.visit_expr(&expression.expr); for arm in &expression.arms { if has_test_cfg(&arm.attrs) { continue; } self.push_pattern(&arm.pat, default_tainted, manager_tainted); visit::visit_arm(self, arm); self.pop_pattern(); } }
    fn visit_expr_for_loop(&mut self, expression: &'ast syn::ExprForLoop) { let default_tainted = uses_default_or_local(&expression.expr, &self.default_locals, &self.default_macros); let manager_tainted = manager_place(&expression.expr, &self.manager_fields, &self.manager_locals); self.visit_expr(&expression.expr); self.push_pattern(&expression.pat, default_tainted, manager_tainted); self.visit_block(&expression.body); self.pop_pattern(); }
    fn visit_expr_if(&mut self, expression: &'ast syn::ExprIf) { let Expr::Let(binding) = &*expression.cond else { visit::visit_expr_if(self, expression); return; }; let default_tainted = uses_default_or_local(&binding.expr, &self.default_locals, &self.default_macros); let manager_tainted = manager_place(&binding.expr, &self.manager_fields, &self.manager_locals); self.visit_expr(&expression.cond); self.push_pattern(&binding.pat, default_tainted, manager_tainted); self.visit_block(&expression.then_branch); self.pop_pattern(); if let Some((_, otherwise)) = &expression.else_branch { self.visit_expr(otherwise); } }
    fn visit_expr_while(&mut self, expression: &'ast syn::ExprWhile) { let Expr::Let(binding) = &*expression.cond else { visit::visit_expr_while(self, expression); return; }; let default_tainted = uses_default_or_local(&binding.expr, &self.default_locals, &self.default_macros); let manager_tainted = manager_place(&binding.expr, &self.manager_fields, &self.manager_locals); self.visit_expr(&expression.cond); self.push_pattern(&binding.pat, default_tainted, manager_tainted); self.visit_block(&expression.body); self.pop_pattern(); }
...
fn boundary_violations(source: &str, module_path: &[&str], manager_fields: &[String]) -> Result<Vec<String>, syn::Error> {
    let file = syn::parse_file(source)?; let default_macros = default_macro_names(&file);
    let mut visitor = BoundaryVisitor {
        module_path: module_path.iter().map(|segment| (*segment).to_string()).collect(), context: Vec::new(), violations: Vec::new(), manager_fields: manager_fields.to_vec(), default_locals: Vec::new(), manager_locals: Vec::new(), local_scopes: Vec::new(), manager_returning: false, return_variants: Vec::new(), default_macros, alias_scopes: Vec::new(), };
...
#[rustfmt::skip] fn provider_runtime_http_boundary_guard_rejects_forbidden_spellings() {
    ...
    assert!(allowed.is_empty(), "allowed fixture violations: {allowed:?}");
    for bypass in [
