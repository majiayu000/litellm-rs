...
#[rustfmt::skip] impl<'ast> Visit<'ast> for ReturnDefaultFinder<'_> { fn visit_expr(&mut self, expr: &'ast Expr) { if !expr_attrs(expr).is_some_and(has_test_cfg) { visit::visit_expr(self, expr); } } fn visit_path(&mut self, path: &'ast Path) { if path.segments.len() == 1 { self.0 |= path.segments.first().is_some_and(|segment| self.2.contains(&ident_text(&segment.ident))); } } fn visit_expr_call(&mut self, expression: &'ast syn::ExprCall) { let variant = match &*expression.func { Expr::Path(path) => path.path.segments.last().map(|segment| ident_text(&segment.ident)), _ => None }; if variant.as_ref().is_some_and(|name| matches!(name.as_str(), "Ok" | "Err" | "Some" | "None")) { if variant.is_some_and(|name| self.1.contains(&name)) { self.0 |= expression.args.iter().any(|argument| uses_default_or_local(argument, self.2, self.3)); } } else { self.visit_expr(&expression.func); expression.args.iter().for_each(|argument| self.visit_expr(argument)); } } fn visit_expr_method_call(&mut self, expression: &'ast ExprMethodCall) { self.visit_expr(&expression.receiver); if !matches!(ident_text(&expression.method).as_str(), "map_err" | "inspect_err" | "filter") { expression.args.iter().for_each(|argument| self.visit_expr(argument)); } } fn visit_macro(&mut self, item: &'ast Macro) { let name = item.path.segments.last().map(|segment| ident_text(&segment.ident)); let tokens = item.tokens.to_string(); self.0 |= name.is_some_and(|name| self.3.contains(&name)) || tokens.split(|character: char| !(character == '_' || character.is_alphanumeric())).any(|token| self.3.iter().any(|name| name == token)); } }
...
#[rustfmt::skip] fn collect_default_aliases(tree: &UseTree, prefix: &mut Vec<String>, aliases: &mut Vec<String>) { match tree { UseTree::Path(path) => { prefix.push(ident_text(&path.ident)); collect_default_aliases(&path.tree, prefix, aliases); prefix.pop(); }, UseTree::Name(name) => { let mut path = prefix.clone(); path.push(ident_text(&name.ident)); if path.len() == 3 && path[0] == "std" && path[1] == "default" && path[2] == "default" { aliases.push(ident_text(&name.ident)); } }, UseTree::Rename(rename) => { let mut path = prefix.clone(); path.push(ident_text(&rename.ident)); if path.len() == 3 && path[0] == "std" && path[1] == "default" && path[2] == "default" { aliases.push(ident_text(&rename.rename)); } }, UseTree::Group(group) => group.items.iter().for_each(|tree| collect_default_aliases(tree, prefix, aliases)), _ => {} } }
#[rustfmt::skip] fn default_macro_names(file: &syn::File) -> Vec<String> { struct Defaults { aliases: Vec<String>, macros: Vec<(String, String)> } impl<'ast> Visit<'ast> for Defaults { fn visit_item_use(&mut self, item: &'ast ItemUse) { collect_default_aliases(&item.tree, &mut Vec::new(), &mut self.aliases); } fn visit_item_macro(&mut self, item: &'ast ItemMacro) { if !has_test_cfg(&item.attrs) && let Some(name) = &item.ident { self.macros.push((ident_text(name), production_macro_tokens(&item.mac.tokens.to_string()))); } } } let mut found = Defaults { aliases: Vec::new(), macros: Vec::new() }; found.visit_file(file); let mut defaults = found.aliases; loop { let mut changed = false; for (name, tokens) in &found.macros { let compact = tokens.replace(' ', "").to_ascii_lowercase(); let identifiers = || tokens.split(|character: char| !(character == '_' || character.is_alphanumeric())); let direct = compact.contains("default::default") || identifiers().any(|token| defaults.iter().any(|default| default == token)); if direct && !defaults.contains(name) { defaults.push(name.clone()); changed = true; } } if !changed { return defaults; } } }
#[rustfmt::skip]
...
#[rustfmt::skip] struct BoundaryVisitor {
    ...
    manager_returning: bool, return_variants: Vec<String>, default_macros: Vec<String>,
}
...
fn boundary_violations(source: &str, module_path: &[&str], manager_fields: &[String]) -> Result<Vec<String>, syn::Error> {
    ...
    let mut visitor = BoundaryVisitor {
        ...
        context: Vec::new(), violations: Vec::new(), manager_fields: manager_fields.to_vec(), default_locals: Vec::new(), local_scopes: Vec::new(), manager_returning: false, return_variants: Vec::new(), default_macros,
    };
